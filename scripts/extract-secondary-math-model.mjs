...
if (!Number.isInteger(startPage) || !outputPath) {
  throw new Error("الاستخدام: node scripts/extract-secondary-math-model.mjs <صفحة-بداية> <مسار-الناتج>");
}

const sourceDirectory = "/home/ubuntu/imports/secondary-math/pages";
...
const response = await fetch(`${process.env.OPENAI_API_BASE}/chat/completions`, {
  ...
  messages: [{
    ...
    content: [
      {
        type: "text",
        text: `هذه ثلاث صفحات متتالية من نموذج رياضيات للثانوية العامة. الصفحة الأولى والثانية تحتويان الأسئلة 1 إلى 40، والصفحة الثالثة تحتوي جدول الإجابة الصحيحة.

انسخ نص كل سؤال وخياراته كما يظهر بصريًا، بما في ذلك الرموز الرياضية، من دون تصحيح لغوي أو حل السؤال من عندك. لكل سؤال اربط rawCorrectAnswer برقم الإجابة الصحيحة الظاهر في جدول الإجابات فقط. لا تستنتج إجابة من الرياضيات إذا لم يكن جدول الإجابات واضحًا.

بالأسئلة صح/خطأ: اكتب الخيارات بالترتيب الظاهر في ورقة الاختبار، وحدد في sourceAnswerEncoding معنى ترميز جدول الإجابة إن كان مرئيًا. في الاختيار من متعدد: اكتب الخيارات بالترتيب من 1 إلى 4 كما تظهر. اجعل needsReview=true لأي سؤال أو رمز أو خيار غير مقروء، واكتب سبب ذلك في unresolvedNotes. الناتج JSON فقط وفق المخطط.`
      },
