...
const subjectKey = "exam_secondary_history";
const sourceDirectory = "/home/ubuntu/imports/secondary-history";
const modelFiles = [1, 2, 3, 4, 5, 6].map(index => `${sourceDirectory}/model-${index}-extracted.json`);

const letterFor = answerIndex => ["A", "B", "C", "D"][answerIndex - 1];

function formNameFor(index) {
  return `النموذج ${["الأول", "الثاني", "الثالث", "الرابع", "الخامس", "السادس"][index - 1]} — تاريخ أدبي`;
}
...
function loadModels() {
  return modelFiles.map((path, index) => {
    ...
    if (!Array.isArray(questions) || questions.length !== 50) {
      throw new Error(`النموذج ${index + 1} لا يحتوي خمسين سؤالًا مكتملًا.`);
    }
...
async function importExams() {
  ...
  try {
    ...
    for (const model of models) {
      const formKey = `secondary_history_model_${model.index}`;
      ...
      for (const question of model.questions) {
        ...
        await connection.execute(
          ...
          [
            `secondary_history_model_${model.index}_q${question.number}`,
            subjectKey,
            formKey,
            question.stem,
    ...
    console.log(JSON.stringify({ subjectKey, forms: models.length, questions: models.length * 50 }, null, 2));
  } catch (error) {
...
importExams().catch(error => {
  console.error(`فشل استيراد نماذج تاريخ الثانوية: ${error instanceof Error ? error.message : String(error)}`);
  process.exitCode = 1;
});
